...
function LandingIndexPage() {
  return (
    <>
      <section className="px-4 pb-24 pt-6 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2 rounded-full border border-background/10 bg-background/[0.06] px-3 py-1 text-sm text-background/85">
              <Sparkles className="size-4" />
              Watch rooms and creator tools in one flow
            </div>

            <div className="space-y-6">
              <h1 className="max-w-4xl text-5xl font-semibold tracking-tight text-background sm:text-6xl lg:text-7xl">
                Launch shared video spaces that feel ready from the first click.
              </h1>
              <p className="max-w-2xl text-lg leading-8 text-background/70">
            ...
            <div className="grid max-w-xl grid-cols-3 gap-3 pt-4">
              {stats.map((stat) => (
                <div key={stat.label} className="border-l border-background/15 pl-4">
                  <div className="text-2xl font-semibold text-background">{stat.value}</div>
                  <div className="mt-1 text-sm text-background/55">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative">
            <div className="overflow-hidden rounded-lg border border-background/10 bg-background/[0.06] shadow-2xl shadow-foreground/30">
              ...
              <div className="grid gap-0 lg:grid-cols-[1.28fr_0.72fr]">
                <div className="min-h-[24rem] bg-[linear-gradient(135deg,var(--background)_0%,var(--secondary)_34%,var(--accent)_68%,var(--primary)_100%)] p-5">
                  <div className="flex h-full flex-col justify-between rounded-lg border border-background/40 bg-background/25 p-5 backdrop-blur-sm">
                    <div className="flex items-center justify-between">
                      <div className="h-3 w-28 rounded-full bg-foreground/40" />
                      <div className="h-8 w-8 rounded-full bg-background/80" />
                    </div>
                    <div className="space-y-3">
                      <div className="h-4 w-48 rounded-full bg-foreground/55" />
                      <div className="h-3 w-64 max-w-full rounded-full bg-foreground/35" />
                      <div className="h-3 w-40 rounded-full bg-foreground/30" />
                    </div>
                  </div>
                </div>

                <div className="space-y-4 bg-foreground/70 p-5">
                  <div className="rounded-lg border border-background/10 bg-background/[0.06] p-4">
                    ...
                    <div className="mt-4 space-y-2">
                      ...
                      <div className="h-2 w-4/5 rounded-full bg-background/15" />
                    </div>
                  </div>
                  <div className="rounded-lg border border-background/10 bg-background/[0.06] p-4">
                    <div className="flex items-center gap-2 text-sm font-medium text-background">
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-2">
                      <div className="h-16 rounded-md bg-primary/70" />
                      <div className="h-16 rounded-md bg-accent/70" />
                      <div className="h-16 rounded-md bg-secondary/70" />
                    </div>
                  </div>
                  <div className="rounded-lg border border-background/10 bg-background/[0.06] p-4">
                    <div className="flex items-center gap-2 text-sm font-medium text-background">
                    ...
                    <div className="mt-4 h-10 rounded-full bg-background text-center text-sm font-medium leading-10 text-foreground">
                      Publish
                    </div>
                  </div>
                </div>
      ...
      <section id="workflow" className="bg-card px-4 py-24 text-foreground sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.72fr_1fr] lg:items-start">
          ...
          <div className="grid gap-4">
            {workflow.map((item, index) => (
              <div
                key={item}
                className="grid gap-4 rounded-lg border border-border bg-muted/45 p-5 sm:grid-cols-[auto_1fr] sm:items-center"
              >
      ...
      <section id="launch" className="px-4 py-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl rounded-lg border border-background/10 bg-background/[0.06] px-6 py-12 text-center shadow-2xl shadow-foreground/20 sm:px-10">
          <h2 className="text-3xl font-semibold tracking-tight sm:text-5xl">
